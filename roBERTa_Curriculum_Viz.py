...
model_name = 'roberta-base'  # Or 'roberta-large' for a larger model if resources allow
tokenizer = RobertaTokenizer.from_pretrained(model_name)
model = RobertaModel.from_pretrained(model_name)

exclude_words = [r'\bEMSC\d* L\d+ Overview\b', r'\bL\d+ Poster\b', r'\bL\d+ Exam\b']  # Add wildcard pattern
...
cluster_groups = [
    ...
    # ... add more groups as needed for 4000 level etc
]
...
for index, row in cluster_relationships_df.iterrows():
    ...
    # Get node TEXTS (not embeddings) for source and target clusters
    ...
    for src_node in source_nodes:
        for tgt_node in target_nodes:
            ...
            if similarity_score >= threshold:
                  ...
                  # Add the nodes to texts_for_clustering as well
                  texts_for_clustering.extend([src_node, tgt_node])
...
if texts:  # Check if texts list is not empty
  embeddings_2d = get_embeddings_batch(texts_for_clustering).numpy()
...
for src, tgt, similarity_score in intercluster_edges:
   # Check if src or tgt contains any excluded words
    if any(re.search(pattern, src) for pattern in exclude_words) or any(re.search(pattern, tgt) for pattern in exclude_words):
        print(f"Skipping edge ({src}, {tgt}) due to excluded words.")
        continue
    ...
    edge_label=f"{similarity_score:.2f} ({src_category}, {tgt_category})"
    dot.edge(str(src), str(tgt), color=edge_color, xlabel=edge_label, style='dashed')
...
with dot.subgraph(name='cluster_legend') as legend_subgraph:
    ...
    legend_subgraph.attr(fixedsize='true', clusterrank='local')
...
output_file = f'curriculum_viz_BART_{timestamp}'
